...
def build_forwarded_request(request_line: str, headers, body: bytes, client_ip: str) -> bytes:
    ...
    filtered_headers.append(("Via", "1.1 python-reverse-proxy"))
...
def handle_client(conn: socket.socket, addr):
    try:
        ...
        print(f"[PROXY] {addr[0]}:{addr[1]} -> {backend_host}:{backend_port} | {request_line}")
        response = forward_request_to_backend(backend_host, backend_port, forwarded_request)
        conn.sendall(response)

    except Exception as exc:
        conn.sendall(build_error_response(400, "Bad Request", f"Erro no reverse proxy: {exc}"))
    finally:
        conn.close()
