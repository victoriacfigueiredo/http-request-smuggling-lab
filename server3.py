...
def parse_authority(authority: str, default_port: int = 80) -> tuple[str, int]:
    if not authority:
...
def classify_request_target(req_line: RequestLine):
    ...
    if target.startswith("http://"):
        ...
        if parsed.scheme != "http":
            raise HTTPError(400, "Bad Request", "Somente http:// suportado neste servidor de lab")
...
def validate_hop_by_hop_policy(headers: list[tuple[str, str]]):
    ...
    if has_header(headers, "upgrade"):
        raise HTTPError(426, "Upgrade Required", "Upgrade não implementado neste servidor de lab")
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        server.listen(50)
        print(f"[Server C] ouvindo em {host}:{port}")
        while True:
